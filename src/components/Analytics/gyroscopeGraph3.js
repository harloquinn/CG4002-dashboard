...
export default class GyroscopeComponent3 extends Component {
  constructor() {
    super();
    this.state = {
      timeArray: [0],
  ...
  render() {
    const GyroscopeComponent3 = createPlotlyComponent(Plotly);

    const socket = socketIOClient(this.state.endpoint);

    const {timeArray,arrayx,arrayy,arrayz} = this.state;
    socket.on('incoming data3', (packet) => {
      timeArray.push(packet[6]);
      arrayx.push(packet[3])
      arrayy.push(packet[4]);
      arrayz.push(packet[5]);
    })

    return (
      <AnalyticsStyles>
          <GyroscopeComponent3 className="accelerometer-graph" data={[
